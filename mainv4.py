...
# --- NX-OS Specific Interface Finalizer ---
def _finalize_nxos_interface_data(iface_dict, parsed_data):
    ...
    Extracts mode, vlan, description, and VPC ID if applicable.
    """
...
def _parse_nx_os_config(config_lines):
    ...
    while i < len(config_lines):
        ...
        if line.startswith("hostname"):
            ...
            # A new interface block is starting. `current_interface` would have been cleared above if active.
            current_interface = {"name": line.split()[1], "lines": []}
        ...
        # --- VPC Domain Parsing for NX-OS ---
            ...
            # Add more VPC domain settings here as needed (e.g., graceful-consistency-check)
    ...
    # Finalize the last interface if it was still active after loop
    if current_interface: 
...
def export_to_excel(parsed_data, output_file, collection_date_obj): 
    ...
    for iface in parsed_data["interfaces"]:
        ...
        # Access attributes from the interface dict (now pre-processed by _finalize_nxos_interface_data)
        mode = iface.get("mode", "Unknown")
        vlan = iface.get("vlan", "None")
        description = iface.get("description", "No Description")
        vpc_info_on_interface = f" | VPC: {iface['vpc_id']}" if iface.get("vpc_id") else "" # NEW: VPC ID on interface

        write_row_to_sheet(f"Interface {iface['name']}", f"IP: {ip_info} | Mode: {mode} | VLAN: {vlan} | Desc: {description}{vpc_info_on_interface}") # NEW: Added vpc_info_on_interface
