...
def _finalize_nxos_interface_data(iface_dict, parsed_data):
    ...
    Extracts mode, vlan, description, IP info, and VPC ID if applicable.
    """
    # Extract IP info
...
def _parse_nx_os_config(config_lines):
    ...
    while i < len(config_lines):
        ...
        # 4. Handle closing of `current_vlan_data` context (NEW)
        ...
        # --- NEW: VLAN Parsing for NX-OS (as context) ---
        elif line.startswith("vlan "): 
            if current_vlan_data: # If previous VLAN block was not closed by context clearing
                parsed_data["vlans"].append(current_vlan_data)
            
            vlan_id_match = re.search(r'vlan ([\d,-]+)', line) # Capture ID (and ranges)
...
def export_to_excel(parsed_data, output_file, collection_date_obj): 
    ...
    for iface in parsed_data["interfaces"]:
        ...
        write_row_to_sheet(f"Interface {iface['name']}", f"IP: {ip_info} | Mode: {mode} | VLAN: {vlan} | Desc: {description}{vpc_info_on_interface}")
