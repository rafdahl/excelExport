...
def _parse_ios_xe_config(config_lines):
    ...
    while i < len(config_lines):
        ...
        # Removed 'ip route' from is_new_top_level_command as it's a sub-command for router context
        is_new_top_level_command = any(line.startswith(cmd) for cmd in [
            ...
            # ip router ospf is handled below for specific router configs, NOT as a general top-level command.
        ])
...
def _parse_nx_os_config(config_lines):
    ...
    while i < len(config_lines):
        ...
        # --- Context Clearing Logic ---
        # Removed 'ip router' and 'ip ospf' etc. from is_new_top_level_command
        # These are now considered sub-commands of an interface if active, not block initiators.
        is_new_top_level_command = any(line.startswith(cmd) for cmd in [
